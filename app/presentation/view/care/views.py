...
def get_form():
    try:
        ...
        if request.values['form'] == 'view':
            data = app.application.student_care.prepare_edit_registration_form(request.values['extra'], read_only=True)
            data.update(common)
        elif current_user.is_at_least_supervisor:
            if request.values['form'] == 'edit':
                data = app.application.student_care.prepare_edit_registration_form(request.values['extra'])
                data.update(common)
            elif request.values['form'] == 'add':
                data = app.application.student_care.prepare_add_registration_form()
                data.update(common)
